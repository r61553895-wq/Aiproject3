...
export const AdminPanelModal: React.FC<AdminPanelModalProps> = ({
  ...
  // Economics settings
  const [pricePer1k, setPricePer1k] = useState<number>(35); // in Rubles (₽)
  ...
  const handleRevokeKey = async (code: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/keys/${encodeURIComponent(code)}`, {
        method: 'DELETE',
  ...
  // 1-Click Copy direct activation link
  const copyActivationLink = (code: string) => {
    const origin = window.location.origin;
    const link = `${origin}/?key=${code}`;
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md">
        <motion.div
          ...
          className="relative w-full max-w-5xl h-[90vh] bg-[#0c0f16] border border-white/10 rounded-3xl flex flex-col shadow-2xl overflow-hidden"
        >
          {/* Header */}
          <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between shrink-0 bg-[#080a0f]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded-full bg-white/10 border border-white/20 text-slate-300">
                    Админ
                  </span>
                </div>
          ...
          {!isAuthenticated ? (
            <div className="flex-1 flex items-center justify-center p-6">
              <div className="w-full max-w-sm bg-[#121622] border border-white/10 rounded-3xl p-6 sm:p-8 space-y-5 text-center shadow-xl">
                <div className="w-14 h-14 mx-auto rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-slate-200">
                ...
                <form onSubmit={handleLogin} className="space-y-4 text-left">
                  <div>
                    <label className="block text-xs text-slate-300 mb-1 font-medium">Пароль</label>
                    <input
                      type="password"
                      value={password}
                      onChange={(e) => setPassword(e.target.value)}
                      placeholder="Введите пароль..."
                      className="w-full px-4 py-3 rounded-xl bg-black/50 border border-white/10 focus:border-emerald-400 focus:outline-none text-white text-sm"
                      autoFocus
                    />
                  </div>
                  ...
                  <button
                    type="submit"
                    className="w-full py-3 rounded-xl bg-white hover:bg-slate-200 active:scale-98 text-slate-950 font-bold text-sm transition-all cursor-pointer shadow-md"
                  >
                    Войти в панель
                  </button>
                </form>
            ...
            <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden">
              ...
              <div className="w-full md:w-56 bg-[#090b10] border-r border-white/5 p-3 flex md:flex-col gap-1 overflow-x-auto shrink-0">
                <button
                  onClick={() => setActiveTab('create')}
                  className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-colors shrink-0 cursor-pointer ${
                    activeTab === 'create'
                          ...
                          {createdKeysList.map((key) => (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  type="button"
                                  onClick={() => copyActivationLink(key.code)}
                                  className="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-slate-200 font-medium flex items-center gap-1.5 transition-colors cursor-pointer"
                                  title="Ссылка для клиента с авто-активацией"
                                >
                                  {copiedLink === key.code ? (
                                    <>
                                      <Check className="w-3.5 h-3.5 text-emerald-400" />
                                  ...
                                  <tr key={key.code} className="hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td className="py-3 px-4 text-right space-x-1">
                                      <button
                                        ...
                                        className="p-1.5 rounded-lg text-slate-300 hover:text-white hover:bg-white/10 transition-colors cursor-pointer inline-block"
                                      >
                                        <Link className="w-3.5 h-3.5" />
                                      </button>
                                      <button
                    ...
                    <div className="p-5 rounded-2xl bg-[#0e1219] border border-white/10 space-y-3 text-xs sm:text-sm text-slate-300">
                      ...
                      <ol className="list-decimal list-inside space-y-2 pl-1 leading-relaxed text-slate-400">
                        <li>
                          ...
                          (например, пакет на 50 000 токенов с заметкой клиента).
                        </li>
                        <li>
                          <strong className="text-slate-200">Примите оплату</strong> от покупателя любым удобным
                          способом (на карту, СБП, Telegram Stars, крипто).
                        </li>
                        <li>
                          <strong className="text-slate-200">Скопируйте прямую ссылку</strong> на активацию ключа
                          (кнопка со значком ссылки 🔗) и отправьте её клиенту в Telegram/чат.
                        </li>
                        <li>
                          Клиент переходит по ссылке — и его баланс пополняется автоматически за 1 секунду!
                        </li>
                      </ol>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {users.map((u) => (
                          ...
                          >
                            <div>
                              <div className="font-mono text-white font-semibold">{u.name}</div>
